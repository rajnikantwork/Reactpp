...
import { updateReading } from "../../screen/resources/action";
...
function SwipableDrawer({
  ...
  const { all, new: ne, viewing, viewed } = data;
  const openModal = (id: string, urlType: string) => {
    const cardData = all.find((value: any) => value._id === id);
    if (urlType === "pdf") {
      dispatch(updateReading(cardData));
      setDetailId(id);
      setOpen(true);
    }
  };
  ...
  return (
    <div className={classes.container}>
      ...
      >
        ...
        />
        <Tab
          label={<Typography variant="body2">{filterName3}</Typography>}
          icon={<div>{viewing.length}</div>}
          {...a11yProps(2)}
          className={classes.label}
        />
        <Tab
      ...
      <TabPanel value={value} index={2}>
        {viewing.length ? (
          <React.Fragment>{dataToRender(viewing, viewAllUrl)}</React.Fragment>
        ) : (
          <NoDataFound title={"No Resource Found"} />
        )}
      </TabPanel>
